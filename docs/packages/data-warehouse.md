...
- Pipelines on the client's own sources, a layered model (staging → business logic →
  presentation), and a warehouse that builds and checks itself automatically.
...
- **Tested transformations** — a test at every step, plus automated data-quality
  checks that catch the problem before the monthly meeting does.
...
- **One definition per figure**, pinned down in documentation — the warehouse
  ...
  there is only one number. Reuses the KPI definitions from the
...
| Week 2 | Build: pipelines on the sources, transformations with a test at every step, an automated build with quality checks |
...
- Consumes phase 1 directly: the [Data scan](data-scan.md)'s target model as the
  blueprint, and its KPI definitions as the model's business logic. Week 1 shrinks
  when that document exists.
- The explicit prerequisite-solver for the AI packages: when the foundation
  isn't ready for AI yet, this is the package that fixes it.
